...
Your task is to translate the [CURRENT SENTENCE] into natural, grammatically correct, and academically sound Vietnamese.
...
3. Keep the output clean. Output ONLY the translated Vietnamese text. No explanations, no markdown formatting unless it is present in the source sentence.
"""

REVIEWER_PROMPT = """You are an Academic Translation Reviewer. Your role is to examine the translation of a sentence and verify its quality.
...
1. Accuracy: Does the translation correctly convey the meaning of the original sentence?
...
{{
  ...
  "suggested_correction": "A corrected version of the translation if it failed, otherwise leave blank or repeat the current translation."
}}
"""

STYLE_ALIGN_PROMPT = """You are a Style and Alignment Agent. Your task is to polish the final Vietnamese translation to ensure it matches the layout, style, and formatting of the original English sentence.

Original Sentence: {original_text}
Reviewed Translation: {translated_text}

Guidelines:
1. Preserve markdown formatting (like asterisks for bold/italic, code blocks, etc.) if they were in the original.
...
4. Output ONLY the finalized Vietnamese sentence. No intro, no outro, no code block markers unless they belong to the translation.
"""
